...
class Controller(Generic[Context]):
	def __init__(
		...
		# 	domains=['a8demo.seeyoncloud.com'],
		# )
		# async def apply_meeting_room(params: ApplyMeetingRoomAction, browser: BrowserContext):
		# 	page = await browser.get_current_page()
		...
		# 		if params.apply_button_index is not None:
		# 			# 如果提供了具体的索引，直接使用
		# 			if params.apply_button_index not in selector_map:
		# 				raise Exception(f'Element index {params.apply_button_index} does not exist')
		# 			apply_button = await browser.get_dom_element_by_index(params.apply_button_index)
		# 		else:
		# 			# 自动查找包含"申请会议室"相关文本的按钮
		# 			for index, element in selector_map.items():
		# 				element_text = element.get_all_text_till_next_clickable_element(max_depth=1)
		# 				if any(keyword in element_text for keyword in ['申请会议室', '申请', 'apply', 'book']):
		# 					apply_button = element
		# 					logger.info(f'Found apply meeting room button at index {index}: {element_text}')
		# 					break
					
		# 			# if not apply_button:
		# 			# 	raise Exception('Could not find apply meeting room button automatically')
				
		# 		# 点击申请会议室按钮
		# 		element_handle = await browser.get_locate_element(apply_button)
		# 		if not element_handle:
